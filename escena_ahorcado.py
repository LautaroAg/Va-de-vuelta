...
class Animales(Base): 
    ...
    def crear_elementos(self):
        ...
        primera_letra = pilas.actores.Texto(self.animal[0].upper(),x=pos,y=200,fuente="data/tipografias/American Captain.ttf")
    ...
    def crear_carteles(self):
        ...
        pos = -265
        for i in range(11):
            l = pilas.actores.Texto(self.lista_letras[i],x=pos,y=-150,fuente="data/tipografias/American Captain.ttf")
            l.escala = 2
        ...
        for i in range(11,21):
            l = pilas.actores.Texto(self.lista_letras[i],x=pos,y=-210,fuente="data/tipografias/American Captain.ttf")
            l.escala = 2
            l.aprender(pilas.habilidades.Arrastrable)
            l.color = pilas.colores.cyan
            l.radio_de_colision =10
    ...
    def mostrar_foto(self):
        ...
        self.boton_pista_definicion.desactivar()
        #Se mestra foto
        ruta="data/imagenes/animales/"+ self.animal +'.png'
        foto = pilas.actores.Actor(imagen = ruta)
    ...
    def perdio(self):
        ...
        #Mostramos cual era
        for i in self.grupo_cuadros:
            l = pilas.actores.Texto(i.get_letra(),fuente="data/tipografias/American Captain.ttf",x=i.x,y=i.y)
    ...
    def finalizar_juego(self, ptos = -1):
        ...
        if ptos < 30:
            try:
                ...
                f= open('data/vidas.txt','wb')
                pickle.dump('Error de archivos - Ahorcado',f)
                f.close()
        else:
            try:
                f = open('data/vidas.txt', 'rb')
                ...
                f= open('data/videos.txt','wb')
                pickle.dump('Error de archivos - Ahorcado',f)
                f.close()

        #Opcion para volver
        boton_volver = pilas.interfaz.Boton("Volver al juego", y=-100)
        boton_volver.conectar(self.guardar_datos)
'''
...
pilas.cambiar_escena(Animales(2))

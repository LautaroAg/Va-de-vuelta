...
class Juego(Base):
    ...
    def guardar_datos(self):
      ...
      data[nombre]['casillas_recorridas'] = self.casillas_recorridas
    ...
    def sumar_vidas(self,cant):
      if cant > 0:
        for indice in range(cant):
          if (len(self.vidas_actuales)) == 0:
            self.vidas_actuales.append(pilas.actores.Actor(imagen='data/imagenes/vida.png', x=230,y=320))
          else:
        ...
        cant *= -1
        for indice in range(cant):
          pilas.avisar('Una vida menos')
          ult_pos = len(self.vidas_actuales) - 1
          ult_obj = self.vidas_actuales[ult_pos]
          self.vidas_actuales[ult_pos].eliminar()
          self.vidas_actuales.remove(ult_obj)
    ...
    def actualizar_puntaje(self):
      try:
        ...
        if data <= 0:
          ...
          j = data - len(self.vidas_actuales)
          self.sumar_vidas(j)
      except IOError:
        j = 3
    ...
    def es_final(self):
      if (self.pos == [17, 13]) or (self.pos == [15, 14]):
        return True
      else:
        return False
   ...
   #         self.terminar_juego()
    ...
    def accion_comodin(self):
      def desactivar_todos():
        self.b_ahor.desactivar()
        self.b_ahor.destruir()
        self.b_verd.desactivar()
        self.b_verd.destruir()
        self.b_viol.desactivar()
        self.b_viol.destruir()
        self.b_azul.desactivar()
